...
import { contentFr as contentPrixHammam } from '../../data/articles/prix-hammam-massage-rabat';
import { contentFr as contentBienfaitsHammam } from '../../data/articles/bienfaits-hammam-marocain';
import { contentFr as contentMeilleurSpa } from '../../data/articles/meilleur-spa-rabat-comment-choisir';

const articleContents: Record<string, JSX.Element> = {
  'prix-hammam-massage-rabat': contentPrixHammam,
  'bienfaits-hammam-marocain': contentBienfaitsHammam,
  'meilleur-spa-rabat-comment-choisir': contentMeilleurSpa
};
...
export default function BlogPost() {
  ...
  const article = getArticleBySlug(slug, 'fr');
  ...
  const articleJsonLd = generateArticleJsonLd(article, 'fr');
  const breadcrumbJsonLd = generateBreadcrumbJsonLd([
    { name: "Accueil", item: "/" },
    { name: "Blog", item: "/blog/" },
    { name: article.titleFr }
  ]);

  return (
    <>
      <Seo 
        title={article.seoTitleFr}
        description={article.metaDescFr}
        canonicalPath={`/blog/${article.slugFr}`}
        alternatePaths={[{ lang: 'en', path: `/en/blog/${article.slugEn}` }]}
      ...
      <main className="bg-ivory-50 pt-24 lg:pt-32">
        ...
        <section className="px-5 sm:px-6 lg:px-8 max-w-4xl mx-auto mb-12 text-center">
          <Link to="/blog" className="text-xs font-medium uppercase tracking-wide text-gold-500 mb-6 hover:text-gold-700 transition-colors inline-block">
            ← Retour au journal
          </Link>
          <div className="flex items-center justify-center gap-4 text-xs font-medium uppercase tracking-wide text-ink/60 mb-6">
            <span>{article.categoryFr}</span>
            <span>•</span>
        ...
        <section className="px-5 sm:px-6 lg:px-8 max-w-7xl mx-auto pb-24 lg:pb-32 border-t border-sand-300/20 pt-24">
          <div className="text-center mb-16">
            <h2 className="font-display text-3xl sm:text-4xl text-ink mb-4">À lire également</h2>
            <p className="text-ink/70 font-light">Découvrez d'autres articles sur le bien-être et nos rituels.</p>
          </div>
